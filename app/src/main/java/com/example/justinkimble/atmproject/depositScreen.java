...
public class depositScreen extends AppCompatActivity {
    TextView checkingBal, spendingBal, textView, textView1, textView2;
    ...
    public void confirmClicked(View v) {
        if (checkingBal.isShown()) {
            ...
            if (total > 0) {
                ...
                Intent intent = new Intent(depositScreen.this, Main2Activity.class);intent.putExtra("Checking", "" +newTrash2);
                intent.putExtra("Spending",""+ trash2);
                intent.putExtra("Checking", ""+newTrash2);
                checkingBal.setText(String.valueOf(total));
                Toast.makeText(depositScreen.this, "Transaction Complete! New Balance: $" + String.valueOf(total), Toast.LENGTH_LONG).show();
                setResult(0, intent);
                finish();
            }
            else {
                checkingBal.setText(trash);
                Toast.makeText(depositScreen.this, "Cannot withdraw more than amount in balance.",Toast.LENGTH_LONG).show();
            }

        }

        if (spendingBal.isShown()) {
            ...
            if (total > 0) {
                ...
                Intent intent = new Intent(depositScreen.this, Main2Activity.class);
